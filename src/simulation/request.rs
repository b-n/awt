...
pub struct Request {
    ...
    source: Option<Arc<ClientProfile>>,
    start: usize,
    abandon_tick: usize,
...
impl From<&Arc<ClientProfile>> for Request {
    fn from(client_profile: &Arc<ClientProfile>) -> Self {
        Self {
            ...
            source: Some(client_profile.clone()),
            ..Self::default()
        }
    }
}
...
mod tests {
    ...
    fn enqueued_request(start: usize) -> (Request, usize) {
        ...
        let abandon_tick = request.set_start(start);
        request.enqueue(start);
        (request, abandon_tick)
    }
